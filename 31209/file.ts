...
class RayTracer {
  ...
  render(scene, ctx, screenWidth, screenHeight) {
    let getPoint = (x, y, camera) => {
      let recenterX = x => (x - screenWidth / 2) / 2 / screenWidth;
      let recenterY = y => -(y - screenHeight / 2) / 2 / screenHeight;
      return Vector.norm(
        Vector.plus(
          camera.forward,
          Vector.plus(
...
function defaultScene(): Scene {
  return {
    ...
    lights: [
      ...
      { pos: new Vector(0, 3.5, 0), color: new Color(0.21, 0.21, 0.35) },
    ],
